                        ...
                        <div class="card-header row m-0" style="padding: 15px 0px 15px 5px">
                            ...
                            <div class="col-8 canphai">
                                @if (!empty($phieuNhapCanXem))
                                    <button class="btn btn-primary btn-round mr-2" onclick="window.open('{{ url('inphieunhap?mapn=' . $phieuNhapCanXem->maphieunhap) }}');">
                                                        ...
                                                        @if (!empty($danhSachChiTietPhieuNhapCanXem))
                                                            ...
                                                            @foreach ($danhSachChiTietPhieuNhapCanXem as $ctpn)
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        @if (!empty($danhSachSanPham))
                                                                            @foreach ($danhSachSanPham as $sanPham1)
                                                                                @if ($sanPham1->masanpham==$ctpn->masanpham)
                                                                                    <input list="chitietphieunhap{{$stt}}" name="chiTietPhieuNhap[]" class="form-control" value = "SP{{ $sanPham1->masanpham }} | {{ $sanPham1->tensanpham }}" required>
                                                                    ...
                                                                    <td>
                                                                        <input class="form-control cangiua" style="padding-left:11px;padding-right:5px;" type="number" pattern="[0-9]*" step="1" id="soLuong{{$stt}}" name="soLuong[]" value="{{$ctpn->soluong}}" min="0" onkeyup="tinhTongTien()" required>
                                                                    </td>
                                                                    <td>
                                                                    ...
                                                                    <td>
                                                                        <input class="form-control canphai" type="text" id="thanhTien{{$stt}}" pattern="[0-9,]*" min="0" value="{{$ctpn->dongia*$ctpn->soluong}}" disabled required>
                                                                    </td>
                                                                </tr>
                                                                @php
                                                                    $stt++;
                                    ...
                                    <div class="col-md-6 col-lg-3">
                                        ...
                                        <div class="form-group form-group-default">
                                            <label for="smallSelect">Nhà cung cấp (*)</label>
                                                        <input list="nhaCungCap" name="thongTinNguoiDung" class="form-control" value="ND{{ $nguoiDungCanXem->manguoidung }} | {{ $nguoiDungCanXem->hoten }} | {{ $nguoiDungCanXem->sodienthoai }}" required>
                                                        <datalist id="nhaCungCap">
                                                            @if (!empty($danhSachNhaCungCap))

                                                            @foreach ($danhSachNhaCungCap as $nhaCungCap)
                                                                <option
                                                                    value="ND{{ $nhaCungCap->manguoidung }} | {{ $nhaCungCap->hoten }} | {{ $nhaCungCap->sodienthoai }}">
                                                                </option>
                                                            @endforeach
                                                            @endif
                                        ...
                                        <div class="form-group form-group-default">
                                            ...
                                            @if ($phieuNhapCanXem->ghichu != null)
                                                <textarea name="ghiChu" rows="5" class="form-control">{{ $phieuNhapCanXem->ghichu }}</textarea>
                                            @else
                                        ...
                                        <div class="row m-0 pt-0">
                                            ...
                                            <div class="form-group col-6 canphai p-0">
                                                <input style="padding-left:15px" class="form-control canphai" type="text"
                                                    id="tongTienHien" pattern="[0-9,]*" value="{{ $phieuNhapCanXem->tongtien }}" min="0"
                                                    disabled="false">
                                        ...
                                        <div class="row m-0 pt-0">
                                            ...
                                            <div class="form-group col-6 canphai p-0">
                                                <input style="padding-left:15px" class="form-control canphai"
                                                    ...
                                                    value="{{ $phieuNhapCanXem->tongtien+$phieuNhapCanXem->congno }}" min="0" required>
                                            </div>
                                        </div>
                                        <div class="row m-0 pt-0">
                                            <div class="form-group col-6 ml-auto canphai p-0"
                                            ...
                                            <div class="form-group col-6 canphai p-0">
                                                <input style="padding-left:15px" class="form-control canphai"
                                                    ...
                                                    value="{{$phieuNhapCanXem->congno }}" min="0" disabled>
                                            </div>
                                        </div>
                                        <input name="maPhieuNhapSua" type="number" hidden value="{{$phieuNhapCanXem->maphieunhap }}" required>
                                        @endif
                                    </div>
                                </div>
                            </div>
...
@section('js')
    ...
    <script>
        function themDong() {
            ...
            cotSanPham.innerHTML = '<input list="chitietphieunhap'+stt+'" name="chiTietPhieuNhap[]" class="form-control" required> <datalist id="chitietphieunhap'+stt+'" >@if (!empty($danhSachSanPham)) @foreach ($danhSachSanPham as $sanPham)<option value = "SP{{ $sanPham->masanpham }} | {{ $sanPham->tensanpham }}"></option>  @endforeach @endif </datalist>';
